...
function MaintenanceCard({ request }: { request: MaintenanceRequest }) {
  ...
  const handleReject = (reason: string) => {
    ...
    // In real app, update the request status with reason
  };

  const handleResolve = (notes: string) => {
    toast.success('Maintenance request marked as resolved');
    setShowResolveDialog(false);
    // In real app, update the request status with notes and resolvedAt date
  };
                    ...
                    return (
                      <div key={event.id} className="relative pl-4">
                        ...
                        <p className="text-xs text-muted-foreground">
                          ...
                          {event.actor && ` • ${event.actor === 'tenant' ? 'Tenant' : event.actor === 'landlord' ? 'Landlord' : 'System'}`}
                        </p>
                      </div>
                    );
                  })}
...
export default function LandlordMaintenance() {
  ...
  const inProgressCount = mockMaintenanceRequests.filter((r) => 
