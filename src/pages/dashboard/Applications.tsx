...
import { mockApplications } from '@/data/mockTenant';
      ...
      <CardContent className="p-6">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex-1 space-y-3">
            ...
            {application.notes && (
              <p className="text-sm p-3 bg-muted rounded-lg">{application.notes}</p>
            )}
...
export default function Applications() {
  ...
  const tabs = [
    { value: 'all', label: 'All', count: mockApplications.length },
    { value: 'pending', label: 'Pending', count: mockApplications.filter((a) => a.status === 'pending').length },
    { value: 'under_review', label: 'Under Review', count: mockApplications.filter((a) => a.status === 'under_review').length },
    { value: 'approved', label: 'Approved', count: mockApplications.filter((a) => a.status === 'approved').length },
    { value: 'rejected', label: 'Rejected', count: mockApplications.filter((a) => a.status === 'rejected').length },
  ];
